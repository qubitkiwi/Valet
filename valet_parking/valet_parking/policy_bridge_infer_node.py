...
CROP_SETTINGS = {
    'front_cam': (0, 480, 0, 640),  # 예: 위 100px, 아래 80px 자름 (가로는 그대로)
    'rear_cam':  (0, 480, 0, 640),  # 예: 후방 카메라는 더 좁게 자름
    'left_cam':  (0, 300, 100, 640),   # 예: 사이드미러 등 불필요 영역 제거
...
def preprocess_like_train(img_bgr: np.ndarray, cam_name: str) -> np.ndarray:
    ...
    if cam_name in CROP_SETTINGS:
        ...
        if y2 > y1 and x2 > x1:
            ...
            # 설정 오류 등으로 크기가 0이 되면 None 리턴하거나 원본 사용
            # 여기서는 안전하게 로그 찍고 원본 사용
            pass
...
class PolicyInferNode(Node):
    def __init__(self):
        super().__init__('policy_bridge_infer_node')
        ...
        self.ckpt_path = str(self.declare_parameter('ckpt_path', '/home/hyunii/test_ws/mobilenetv3s_pretrained_up_mid_crop_LR_onecycle_batch256_epoch100_lr0001.pth').value).strip()
        ...
        # pub
        # self.pub_cmd = self.create_publisher(Twist, '/cmd_vel_drive', 10)
        self.pub_ctrl = self.create_publisher(Twist, '/controller/cmd_vel', 10)
    ...
    def tick(self):
        ...
        for cam in self.cams:
            ...
            if bgr is None:
                ...
                self.pub_ctrl.publish(Twist())
                return
            img_chw = preprocess_like_train(bgr, cam)  # (3,224,224) float32 0~255
